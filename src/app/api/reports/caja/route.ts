  ...
  try {
    ...
    if (startParam && endParam) {
      ...
      const perDayRows: Array<{
        ...
        SELECT
          ...
          COALESCE((SELECT SUM(c.monto) FROM "Caja" c WHERE c.fecha >= days.day AND c.fecha < (days.day + interval '1 day') AND c.tipo = 'GASTO' ${Prisma.raw(rutaFilterCaja)}),0)::text as gastos,
          COALESCE((SELECT SUM(c.monto) FROM "Caja" c WHERE c.fecha >= days.day AND c.fecha < (days.day + interval '1 day') AND c.tipo = 'ENTRADA_RUTA' ${Prisma.raw(rutaFilterCaja)}),0)::text as entradas_ruta,
          COALESCE((SELECT SUM(c.monto) FROM "Caja" c WHERE c.fecha >= days.day AND c.fecha < (days.day + interval '1 day') AND c.tipo = 'SALIDA_RUTA' ${Prisma.raw(rutaFilterCaja)}),0)::text as salidas_ruta
        FROM days
        ORDER BY day ASC;
      `;
    ...
    const rows: Array<{
      ...
      SELECT
        ...
        COALESCE((SELECT SUM(c.monto) FROM "Caja" c WHERE date_trunc('day', c.fecha) = days.day AND c.tipo = 'GASTO' ${Prisma.raw(rutaFilterCaja)}),0)::text as gastos,
        COALESCE((SELECT SUM(c.monto) FROM "Caja" c WHERE date_trunc('day', c.fecha) = days.day AND c.tipo = 'ENTRADA_RUTA' ${Prisma.raw(rutaFilterCaja)}),0)::text as entradas_ruta,
        COALESCE((SELECT SUM(c.monto) FROM "Caja" c WHERE date_trunc('day', c.fecha) = days.day AND c.tipo = 'SALIDA_RUTA' ${Prisma.raw(rutaFilterCaja)}),0)::text as salidas_ruta
      FROM days, ref_day
